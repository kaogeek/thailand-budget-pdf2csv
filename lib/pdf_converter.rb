class PDFConverter
  ...
  def extract(start_page, end_page, output_suffix: '', input_path: 'budget-pdf', output_path: 'lib/budget-html', extract_all: false)
  ...
  def extract_all(input_path: 'budget-pdf', output_path: 'lib/budget-html')
    proj_root_path, pdf_dir = gen_dirs(input_path, output_path)
    system extract_all(input)
  end
  ...
  private def extract_cmd(start_page, end_page, proj_path, input_path, output_filename)
    "docker run -ti --rm -v #{proj_path}:/pdf pdf2htmlex/pdf2htmlex:#{IMAGE_VERSION} -f #{start_page} -l #{end_page} --tounicode 1 --optimize-text 1 --space-as-offset 0 --process-outline 0 #{input_path} #{output_filename}"
  end

  private def extract_all_cmd(proj_path, input_path, output_filename)
    "docker run -ti --rm -v #{proj_path}:/pdf pdf2htmlex/pdf2htmlex:#{IMAGE_VERSION} --tounicode 1 --optimize-text 1 --space-as-offset 0 --process-outline 0 #{input_path} #{output_filename}"
  end
end